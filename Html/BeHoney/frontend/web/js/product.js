$(document).ready(function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.success) {
                    ...
                    $('.product__cost').attr('data-priceProduct', data.price);
                    $('#addToShopcartForm').attr('action', '/shopcart/' + data.id + '/add/')
    ...
    $('input[type=radio][name=volume]').change(function() {
        var slug = $('.radioList').attr('data-slug');
    ...
    function pricePackage() {
        var priceProduct = Number($('.product__cost').attr('data-priceProduct'));
        var countPackages = Number($('.count__field').val());
        var price = priceProduct * countPackages;
        $('.productPrice').text(number_format(price, 2, ',', ' '));
    }
